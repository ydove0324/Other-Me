...
export const useAuthStore = create<AuthState>((set) => ({
  ...
  fetchMe: async () => {
    ...
    try {
      const { data: refreshData } = await api.post<ApiResponse<{ access_token: string; refresh_token: string }>>(
        "/auth/refresh",
      ...
      if (refreshData.code === 0 && refreshData.data) {
        ...
        set({ accessToken: refreshData.data.access_token, isAuthenticated: true });

        const { data: meData } = await api.get<ApiResponse<UserInfo>>("/auth/me");
        if (meData.code === 0 && meData.data) {
          set({ user: meData.data, loading: false });
          return;
        }
      }
    } catch {
